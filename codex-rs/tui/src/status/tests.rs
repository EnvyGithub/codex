...
fn sanitize_directory(lines: Vec<String>) -> Vec<String> {
    // 版本号归一化正则: (v0.93.0) -> (v0.0.0)
    let version_re = regex_lite::Regex::new(r"\(v\d+\.\d+\.\d+\)").unwrap();
    lines
        .into_iter()
        .map(|line| {
            // 先归一化版本号
            let line = if line.contains("(v") {
                version_re
                    .replace_all(&line, |caps: &regex_lite::Captures| {
                        let matched = caps.get(0).unwrap().as_str();
                        let padding = " ".repeat(matched.len().saturating_sub("(v0.0.0)".len()));
                        format!("(v0.0.0){padding}")
                    })
                    .to_string()
            } else {
                line
