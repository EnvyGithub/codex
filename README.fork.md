...
协议与字段定义见：[docs/translation.md](docs/translation.md) 的 "外部翻译器协议（stdin / stdout JSON）" 章节。
...
本仓库主要在 **Windows 11 + WSL2** 环境开发/测试。其他环境尚未系统验证，欢迎反馈与 PR。
...
- 请优先在本仓库提 issue
- 若确认/怀疑为上游问题（`openai/codex`），请附上游 issue 链接（或你已检索的关键词与复现步骤），便于跟踪与同步修复
