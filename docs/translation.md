...
本分支为 Codex CLI 增加了一个**可选**的翻译扩展点：通过 `config.toml` 指定一个“外部可执行命令”作为翻译器，把 TUI 中的推理相关输出（`AgentReasoning`）从英文翻译成中文，并以“双语”形式显示。
...
- **不影响 LLM 上下文 / token**：译文仅作为 **TUI 展示层** 追加的 UI 块，不会回写到会话消息历史，也不会进入后续请求的上下文构造与 token 计数逻辑。
  - 注意：若你的外部翻译器本身调用 LLM（例如调用 OpenAI 兼容接口/本地模型），会产生 **翻译器自己的额外 token 消耗**；但这与 Codex 主会话的上下文与 token 预算是两条独立链路。
...
- **TUI**
  ...
  - 推理正文（译文块）：
    ...
    - 为了保证**译文紧跟原文**，TUI 会在译文生成完成（或超时）前，**短暂缓冲后续的历史输出**，避免其它块插入导致错位。
...
- **非 TUI 输出**
  - 当前实现仅在 TUI 展示译文；其它输出渠道保持上游行为（不翻译、不改变输出）。
